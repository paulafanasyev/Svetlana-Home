...
| Компонент | Версия | Лицензия |
|-----------|--------|----------|
| llama-android (dev.ffmpegkit-maintained) | 0.1.1 | MIT |
| llama.cpp / ggml (внутри AAR) | upstream commit 2da6686 | MIT |

Назначение: on-device LLM inference для GGUF-моделей. Библиотека поставляет
нативные библиотеки только для `arm64-v8a` (CPU/NEON). Сама библиотека
**не скачивает модели** — файлы GGUF устанавливаются пользователем явно (ТЗ §87).
