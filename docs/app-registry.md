...
Список реален — строится из `PackageManager`, а не из зашитого перечня:

- `ServiceLocator.init()` вызывает `AppRegistry.scanAsync()` при старте
  приложения (в фоновом потоке, не блокируя запуск).
- `AppDrawerActivity` при открытии вызывает `scan()`, обновляя состав.
- `scan()`persistит результат в `app_registry.json`.
- `startWatching()` регистрирует приёмник `ACTION_PACKAGE_ADDED` /
  `REMOVED` / `REPLACED` / `CHANGED` и обновляет реестр в реальном времени.

Без вызова `scan()` drawer был бы пустым — это был P0-баг аудита
(сканирование объявлялось, но нигде не вызывалось).
...
| Проверка | Где | Статус |
|----------|-----|--------|
| `scan()` строит реальный список из PackageManager | `AppRegistryDeviceTest` | CODE VERIFIED |
| Drawer показывает счетчик установленных приложений | `AppDrawerActivity` | CODE VERIFIED |
